...
$target_file = '/send_paamelding.php';
...
//   $fields[$key] = $value['fieldtitle'];
//}

// testet ut av Eskil, ser ut til å virke
...
//$paamelding = array_merge($fields, $inlinebox, $box);
// Fjernet i prinsippet array mergen
#$paamelding = $fields;

// Style definitions (for style="..." parameters)
function s_td($extra = "") {
	$style_td = 'vertical-align:top; padding-left:7px; padding-right:7px;';
	return 'style="'.$style_td.$extra.'"';
}

$filename = 'tmp/registration.html';
$fp = fopen($filename, w);
...
<h1 style="margin-top: 2em;margin-bottom: 1em;" align="center">'.$spillnavn.'<br>'.$LANG['MISC']['registration'].'</h1>
...
<table border="0" align="center">
	<tr>
		...
		<td '.s_td().'><input type="text" tabindex="1" size="20" id="fornavn" name="person[fornavn]" value="">
'.$mand_mark.'</td>
	...
	<tr>
		...
		<td '.s_td().'><input type="text" tabindex="2" size="20" id="etternavn" name="person[etternavn]" value="">
		'.$mand_mark.'</td>
	</tr>
	<tr>
		<td '.s_td().'><strong>'.$LANG['MISC']['birthdate'].'</strong></td>
		<td '.s_td().'><select id="dag" tabindex="3" name="person[dag]"><option value="" style="margin-bottom:0.5em; font-style:italic">'.$LANG['MISC']['day'].'</option>'.print_liste($dager, $dag).'</select>
			<select id="mnd" tabindex="4" name="person[mnd]"><option value="" style="margin-bottom:0.5em; font-style:italic">'.$LANG['MISC']['month'].'</option>'.print_liste($mnder, $mnd).'</select>
			<select id="aar" tabindex="5" name="person[aar]"><option value="" style="margin-bottom:0.5em; font-style:italic">'.$LANG['MISC']['year'].'</option>'.print_liste($aarliste, $aar).'</select> '.$mand_mark.'</td>
	</tr>
	<tr>
			...
			switch ($fieldinfo['type']) {
				...
				fwrite($fp, '
						<tr>
							...
							<td '.s_td('text-align: right; white-space: nowrap').'><input type="text" tabindex="'.$tabindex.'" id="mal_'.$fieldname.'" name="paamelding['.$fieldname.']" value="" size="'.$extras[0].'"></td>
				...
				case 'inlinebox':
					fwrite($fp, '
						...
						<tr>
							...
							<td '.s_td('text-align: right').'><textarea id="mal_'.$fieldname.'" name="paamelding['.$fieldname.']" tabindex="'.$tabindex.'" cols="'.$extras[1].'" rows="'.$extras[0].'"></textarea></td>
							<td>'); if ($fieldinfo['mand'] == 1) { fwrite($fp, $mand_mark); } else { fwrite($fp, '&nbsp;'); } fwrite($fp, '</td>
						</tr>
					');
					break;
				case 'box':
					fwrite($fp, '
						...
						<tr>
							<td colspan="2" '.s_td('text-align:left; white-space: nowrap').'><strong>'.$fieldinfo['fieldtitle'].'</strong>'); if ($fieldinfo['mand'] == 1) { fwrite($fp, ' '.$mand_mark); } fwrite($fp, '</td>
						</tr>
						<tr>
							<td colspan="2" '.s_td('text-align: center').'><textarea id="mal_'.$fieldname.'" name="paamelding['.$fieldname.']" tabindex="'.$tabindex.'" cols="75" rows="'.$extras[0].'"></textarea></td>
						</tr>
						<tr>
							<td '.s_td('text-align:left').'>
				...
				case 'listsingle':
					fwrite($fp, '
						<tr>
							<td '.s_td('text-align:left; white-space: nowrap').'><strong>'.$fieldinfo['fieldtitle'].'</strong></td>
							<td '.s_td('text-align:right; white-space: nowrap').'><select tabindex="'.$tabindex.'" id="mal_'.$fieldname.'" name="paamelding['.$fieldname.']">
								<option value="" style="margin-bottom:0.5em; font-style:italic">- '.$LANG['MISC']['select'].' -</option>');
				...
				case 'listmulti':
					...
					fwrite($fp, '
						<tr>
							<td '.s_td('text-align:left; white-space: nowrap').'><strong>'.$fieldinfo['fieldtitle'].'</strong></td>
							<td '.s_td('text-align:right; white-space: nowrap').'><select tabindex="'.$tabindex.'" id="mal_'.$fieldname.'" name="'.$fieldname.'[]" multiple>');
					for ($i = 1; $i < (int)$extras[0]+1; $i++) {
						fwrite($fp, '<option value="'.$extras[$i].'">'.$extras[$i].'</option>');
					}
					fwrite($fp, '</select></td>
				...
				case 'radio':
					...
					for ($i = 1; $i < (int)$extras[0]+1; $i++) {
						fwrite($fp, '<input type="radio" tabindex="'.$tabindex.'" id="mal_'.$fieldname.'_'.$extras[$i].'" name="paamelding['.$fieldname.']" value="'.$extras[$i].'">'.ucwords(stripslashes($extras[$i])));
					}
					fwrite($fp, '</td>
							<td>'); if ($fieldinfo['mand'] == 1) { fwrite($fp, $mand_mark); } else { fwrite($fp, '&nbsp;'); } fwrite($fp, '</td>
						</tr>
				...
				case 'check':
					fwrite($fp, '
						<tr>
							<td '.s_td('text-align:left; white-space: nowrap').'><strong>'.$fieldinfo['fieldtitle'].'</strong></td>
							<td '.s_td('text-align:right; white-space: nowrap').'><input value="1" tabindex="'.$tabindex.'" id="mal_'.$fieldname.'" name="paamelding['.$fieldname.']" type="checkbox"></td>
						</tr>
					');
					break;
				case 'calc':
				...
				case 'dots':
					...
					for ($i = 1; $i <= $extras[0]; $i++) {
						fwrite($fp, '<input id="mal_'.$fieldname.'_'.$i.'" type="radio" name="paamelding['.$fieldname.']_'.$i.'" value="'.$i.'" onClick="javascript:check_dots(\''.$fieldname.'\', '.$i.', '.$extras[0].')">');
					}
				...
				case 'separator':
						fwrite($fp, '
								<tr>
										<td '.s_td().' colspan="2"><hr size="2"></td>
						</tr>
					');
					break;
			}
